...
//Funciones para las peticiones hacia el backend - cada una debe llevar su propia ruta "/routes/colaborador"
...
if (colaboradores[0].estado) {
    ...
    res.status(200).send({data:undefined,message:'Ya no tiene acceso al panel'});
}
...
//Método para listar el nombre de los colaboradores que su campo rol sea 'Asesor'
const listar_asesores_admin = async function(req,res){
    if(req.user){
        let colaboradores = await Colaborador.find({rol:'Asesor',estado:true}).select('_id fullname nombres apellidos');
        res.status(200).send({data:colaboradores});
    }else{
        res.status(403).send({data:undefined,message:'Notoken'});
    }
...
//estas rutas deben ir en routes/colaborador.js
module.exports = {
