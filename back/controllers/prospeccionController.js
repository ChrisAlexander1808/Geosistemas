...
const crear_correo_prospeccion_admin = async function (req,res) {
    if(req.user){
        ...
        enviar_correo_prospeccion(cliente.razonsocial,data.asunto,cliente.email,data.contenido);
...
const listar_tarea_prospeccion_admin = async function (req,res) {
    if(req.user){
        let id = req.params['id'];
        //nombre del arreglo para almacenar las variables
        let tareas = await Cliente_tarea.find({cliente:id}).populate('asesor').populate('marca_asesor').sort({createAt:-1});
        //para mostrarlo en el Frontend se le asigna a la variable data
...
const marcar_tarea_prospeccion_admin = async function (req,res) {
    if(req.user){
        let id = req.params['id'];
        //nombre del arreglo para actualizar el estado de la variable estado
        let tareas = await Cliente_tarea.findByIdAndUpdate({_id:id},{
            estado: true,
...
const listar_correos_prospeccion_admin = async function (req,res) {
    if(req.user){
        let id = req.params['id'];
        let correos = await Cliente_correo.find({cliente:id}).populate('asesor').sort({createAt:-1});
