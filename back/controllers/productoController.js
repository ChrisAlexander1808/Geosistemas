    ...
    if (req.user) {
        ...
        try {
            ...
            if (productos.length >= 1) {
                res.status(200).send({data:undefined,message:'Ya existe un producto con ese nombre'})
            } else {
...
const eliminar_variedad_producto_admin = async function(req,res){
    if(req.user){
        ...
        if (variedad.stock == 0) {
            await Variedad.findById({_id:id});
            res.status(200).send({data:variedad}); 
        }else{
...
const obtener_variedad_producto_admin = async function(req,res){
    if(req.user){
        let id = req.params['id'];
...
const registrar_inventario_admin = async function(req,res){
    if(req.user){
        ...
        let nuevo_precio = data.costo_unitario;
        ...
        let monto_ganancia = Math.round((data.costo_unitario*margen_ganancia)/100); //100*30 = 3000/100 = 30
        let nuevo_precio_producto = data.costo_unitario + monto_ganancia;
...
const obtener_inventario_admin = async function(req,res){
    if (req.user) {
        ...
        let variedades = await Variedad.find({stock: {$gte:1}}).select('_id sku titulo stock precio').populate({
            path: 'producto',
...
const obtener_inventario_entrada_admin = async function(req,res){
    if (req.user) {
        try {
            ...
            const inventario = await Inventario.find().select('_id cantidad costo_unitario createdAt proveedor').populate({
                path: 'producto',
                select: '_id titulo'
            }).populate({
                path: 'variedad',
            ...
            const filteredInventario = inventario.filter(element => {
                ...
                res.status(200).send({data:filteredInventario});         
...
const obtener_inventario_salida_admin = async function(req,res){
    if (req.user) {
        try {
            ...
            const venta_detalle = await Venta_detalle.find().select('cantidad precio creatdAt').populate({
                path: 'producto',
                select: '_id titulo'
            }).populate({
                path: 'variedad',
                ...
                path: 'cliente',
                select: '_id nombrecomercial'
            });    
      
            const filteredInventario = venta_detalle.filter(element => {
                let date = new Date(element.creatdAt);
                let _year = date.getFullYear();
                let _month = date.getMonth()+1;
...
module.exports = {
    ...
    obtener_variedad_producto_admin,
