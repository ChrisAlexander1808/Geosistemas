...
@Component({
  ...
  styleUrls: ['./index-gastos.component.css']
})
export class IndexGastosComponent implements OnInit {
  ...
  public imprimirdetalle : Array<any>= [];
    ...
    init_proveedores(){
      this._gastoService.listar_proveedores_admin(this.token).subscribe(
        response=>{
    ...
    init_compras_fecha(){
      this.load_data = true;
      this._gastoService.obtener_gastos_fechas(this.inicio,this.hasta,this.proveedor,this.token).subscribe(
        response=>{
          this.compras = [];
            for(var item of response.data){
              const proveedorNombre = item.proveedor ? item.proveedor.nombrecomercial : 'Sin proveedor';
    ...
    init_compras_hoy(){
      this.load_data = true;
      this._gastoService.obtener_gastos_hoy(this.token).subscribe(
        response=>{
          this.compras = [];
            for(var item of response.data){
              const proveedorNombre = item.proveedor ? item.proveedor.nombrecomercial : 'Sin proveedor';
    ...
    filtrar(){
      if (!this.inicio) {
        ...
        this._router.navigate(['/gastos'],{queryParams: {inicio:this.inicio,hasta:this.hasta,proveedor:this.proveedor}});
      }
    }
    ...
    openEditModal(idx:any) {
      ...
      this._gastoService.obtener_gasto_admin(idx,this.token).subscribe(
        response=>{          
