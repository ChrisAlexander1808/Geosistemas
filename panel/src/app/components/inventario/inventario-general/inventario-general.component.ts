...
export class InventarioGeneralComponent implements OnInit {
    ...
    search(){
        let term_text = new RegExp(this.filtro_texto,'i');
        let term_tipo:any;
        ...
        for(var item of this.inventario_const){
          ...
          if (this.filtro_tipo == 'Todos') {
            if (this.filtro_categoria == 'Todos') {
                if (term_text.test(item.sku)||term_text.test(item.producto)) {
    ...
    downloadExcel(){
      ...
      worksheet.columns = [
        ...
        {header: 'UBICACION', key: 'col4', width: 25},
        {header: 'UNIDAD DE MEDIDA', key: 'col5', width: 25},
    ...
    toCSV(){
      if (this.inventario.length >= 1) {
        const csvExportar = generateCsv(this.inventario);
